...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [viewsReal, setViewsReal] = useState<number>(0);
  ...
  useEffect(() => {
    async function fetchData() {
        try {
            const [categoriesRes, contextRes, postRes] = await Promise.all([
                fetch("/api/categories").then(r => r.json()),
                fetch("/api/post-editor-context").then((r) => (r.ok ? r.json() : null)),
                fetch(`/api/posts/${id}`).then(r => r.json())
            ]);

            setCategories(categoriesRes);
            setWatermarkSettings(contextRes?.watermarkSettings || null);
            setUsers(Array.isArray(contextRes?.users) ? contextRes.users : []);
            ...
            if (postRes) {
                  ...
                  Array.isArray(post.tags)
                    ? post.tags
                        .map((t: any) => {
                          ...
                          if (typeof t?.tag?.name === "string") return t.tag.name;
                          return "";
                        })
                ...
                setViewsBase(typeof post.viewsBase === "number" && Number.isFinite(post.viewsBase) ? Math.max(0, Math.floor(post.viewsBase)) : 0);
                setViewsReal(typeof post.views === "number" && Number.isFinite(post.views) ? Math.max(0, Math.floor(post.views)) : 0);
                ...
                const initialIdsRaw: string[] = Array.isArray(post.postCategories)
                  ? post.postCategories
                      .map((pc: any) => pc?.categoryId || pc?.category?.id)
                      .filter((v: any) => typeof v === "string" && v.trim() !== "")
                  : [];
                const initialIds = Array.from(new Set([...(initialIdsRaw || []), ...(post.categoryId ? [post.categoryId] : [])]));
                setSelectedCategoryIds(initialIds);
                
                setAuthorId(post.authorId || "");
                setApprovedById(post.approvedById || "");
                setReviewEditorIds(Array.isArray(post.reviewEditorIds) ? post.reviewEditorIds : []);

                // Media
                setFeaturedImageId(post.featuredImageId || "");
                setThumbnailImage(post.image || "");
                setFeaturedPreviewUrl(post.featuredImage?.fileUrl || "");
                setFeaturedImageRemoved(false);
                setFeaturedImageAlt(post.featuredImageAlt || "");
                setPostImageWatermarkEnabled(post.postImageWatermarkEnabled === true);
                setImageCaption(post.imageCaption || "");
                
                if (post.gallery && Array.isArray(post.gallery)) {
                    setGallery(post.gallery.map((g: any, index: number) => {
                        if (typeof g === "string") {
                            return { id: g, url: g, caption: "" };
                        }
                        const url = typeof g?.url === "string" ? g.url : "";
                        const id = typeof g?.id === "string" && g.id.trim() !== "" ? g.id : (url || String(index));
  ...
  return (
    <div className="admin-post-editor-page px-3 py-4 sm:px-4 sm:py-5 md:p-8 admin-form bg-[var(--bg-base)] min-h-screen relative">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-bold text-[var(--fg-primary)] sm:text-2xl">Edit Berita</h1>
        ...
        <div className="space-y-5 lg:space-y-6">
            ...
            <div className="card p-4 sm:p-5">
              ...
              <div className="mt-2 text-xs text-[var(--fg-muted)]">
                Total tampil saat ini: {Math.max(0, Math.floor((viewsReal || 0) + (viewsBase || 0)))} (views sistem {viewsReal || 0} + awal {viewsBase || 0})
              </div>
            </div>
              ...
              <label className="mt-3 flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-3 py-3">
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-1 block text-xs text-[var(--fg-muted)]">
                    Toggle ini tersimpan per artikel, jadi berita lama bisa Anda buka kembali lalu aktifkan watermark-nya.
