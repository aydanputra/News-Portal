...
export function useHomepageBuilder() {
  ...
  useEffect(() => {
    async function initData() {
        try {
            ...
            const resBlocks = await fetch(`/api/homepage?themeId=${currentTheme}`);
  ...
  const resetAllSettings = useCallback(async () => {
      if (confirm("PERINGATAN: Ini akan mereset SELURUH konfigurasi homepage ke default awal. Semua perubahan akan hilang. Lanjutkan?")) {
          setLoading(true);
      try {
