...
import { Layout, Palette, Monitor, Tablet, Smartphone, X, Image as ImageIcon } from "lucide-react";
import { ColorPickerWithOpacity } from "../../../../components/admin/page-builder/ColorPickerWithOpacity";
...
interface SectionConfigPanelProps {
    ...
    uiContext?: "home" | "archive" | "header" | "footer";
    section: Block;
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
                {activeSectionTab === "layout" ? (
                    <div className="space-y-6">
                        <div>
                            <label className="text-xs font-bold text-[var(--fg-primary)] uppercase tracking-wider block mb-3">Struktur Kolom</label>
                            <div className="grid grid-cols-3 gap-3">
                                {['100', '50-50', '33-66', '66-33', '33-33-33', '25-25-25-25'].map(layout => (
                                    <button 
                                        key={layout}
                                        onClick={() => updateSectionConfig("layout", layout)}
                        ...
                        {uiContext === "header" && (
                            <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        <label className="text-xs font-bold text-[var(--fg-primary)] uppercase tracking-wider block">Sticky Section</label>
                                        <p className="text-[10px] text-[var(--fg-muted)] mt-1">Section ini akan menempel di atas saat scroll. Anda bisa mengaktifkan lebih dari satu sticky section (akan bertumpuk rapi).</p>
                                    </div>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            className="sr-only peer"
                                            checked={getConfigBool("sticky", false)}
                                            onChange={(e) => updateSectionConfig("sticky", e.target.checked)}
                                        />
                        ...
                        {supportsSidebarSync && !!builderLocation && (
                            <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
                                <div className="flex items-start justify-between gap-3 mb-3">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-[var(--fg-muted)] mt-1">Sidebar pada section ini akan memakai sidebar pertama dari halaman sumber yang dipilih.</p>
                                    </div>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                ...
                                {getConfigBool("followSharedSidebar", false) && (
                                    <div className="space-y-3">
                                        ...
                                        <p className="text-[10px] text-[var(--fg-muted)]">Widget di kolom sidebar section ini akan diganti otomatis oleh sidebar pertama dari halaman sumber.</p>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        <div className="flex bg-[var(--bg-base)] p-1 rounded-lg w-fit border border-[var(--border)]">
                            {devices.map((device) => (
                                <button
                                    key={device}
                                    onClick={() => setActiveSectionDeviceTab(device)}
                                    className={`p-1.5 rounded-md transition-all ${activeSectionDeviceTab === device ? "bg-[var(--bg-elevated)] text-[var(--accent)] shadow-sm" : "text-[var(--fg-muted)] hover:text-[var(--fg-primary)]"}`}
                                    title={device}
                                >
                        ...
                        {/* Gap Settings */}
                        <div>
                            <label className="text-xs font-bold text-[var(--fg-primary)] uppercase tracking-wider block mb-3">Lebar Konten - {activeSectionDeviceTab}</label>
                        ...
                        <div>
                            <label className="text-xs font-bold text-[var(--fg-primary)] uppercase tracking-wider block mb-3">Gap (Jarak) - {activeSectionDeviceTab}</label>
                            ...
                            <div className="mb-4">
                                ...
                                <input 
                                    ...
                                    max="12" 
                                    step="1"
                                    value={getConfigNumber("blockGap") ?? 6}
                            ...
                            {/* Horizontal Gap (Between Columns / Sidebar) */}
                            <div className="mb-4">
                                <div className="flex justify-between mb-1">
                                ...
                                <input 
                                    ...
                                    max="12" 
                                    step="1"
                                    value={getConfigNumber("columnGap") ?? 6}
                        ...
                        {/* Style Tab Content - Now Theme Aware */}
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
                            <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Background & Padding</label>
                            
                            {/* Device Tabs for Style */}
                            <div className="flex bg-[var(--bg-base)] p-1 rounded-lg mb-4 w-fit border border-[var(--border)]">
                                {devices.map((device) => (
                                    <button
                                        key={device}
                                        onClick={() => setActiveSectionDeviceTab(device)}
                            ...
                            <div className="border-t border-[var(--border)] pt-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input 
                                            ...
                                            onChange={(e) => updateSectionConfig("useBox", e.target.checked)}
                                        />
                                        <div className="w-9 h-5 bg-[var(--bg-base)] peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[color:var(--accent)/0.2] rounded-full peer border border-[var(--border)] peer-checked:bg-[var(--accent)] peer-checked:border-[var(--accent)] peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all"></div>
                                    </label>
                                </div>
                                ...
                                <div className="mb-4 animate-in fade-in slide-in-from-top-2">
                                    <div className="mb-3">
                                        <ColorPickerWithOpacity 
                                            ...
                                            label={`${getConfigBool("useBox", false) ? "Warna Background Box" : "Warna Background Section"} (${activeSectionDeviceTab.toUpperCase()})`}
                                        />
                                    </div>
                                </div>
                                ...
                                {getConfigBool("useBox", false) && (
                                    <div className="animate-in fade-in slide-in-from-top-2 duration-200 bg-[var(--bg-base)] p-3 rounded-lg border border-[var(--border)] mb-4">
                                        <div className="mb-3">
                                            <label className="text-[10px] text-[var(--fg-primary)] block mb-1">Border Radius</label>
                                            <select 
                                                className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded p-1.5 text-xs outline-none focus:border-[var(--accent)]"
                                                value={getConfigString("borderRadius", "none")}
                                                onChange={(e) => updateSectionConfig("borderRadius", e.target.value)}
                                            >
                                        ...
                                        <div>
                                            <label className="text-[10px] text-[var(--fg-primary)] block mb-1">Padding Dalam Box</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                <input 
                                                    type="text" 
                                                    placeholder="Vertical (py-)"
                                                    value={getConfigString("boxPaddingY")}
                                                    onChange={(e) => updateSectionConfig("boxPaddingY", e.target.value)}
                                                    className="bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded p-1.5 text-xs outline-none focus:border-[var(--accent)]"
                                                />
                                                <input 
                                                    type="text" 
                                                    placeholder="Horizontal (px-)"
                                                    value={getConfigString("boxPaddingX")}
                                                    onChange={(e) => updateSectionConfig("boxPaddingX", e.target.value)}
                                                    className="bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded p-1.5 text-xs outline-none focus:border-[var(--accent)]"
                                                />
                                            </div>
                                        </div>
                        ...
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
                            <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Background Image Section</label>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-[10px] text-[var(--fg-muted)] mt-2">Pilih gambar dari Image Gallery. Upload file baru bisa dilakukan dari modal gallery.</p>
                                </div>
                            </div>
                            ...
                            {getConfigString("backgroundImage") && (
                                <div className="mt-4 grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="text-[10px] text-[var(--fg-primary)] block mb-1">Overlay Color</label>
                                        <ColorPickerWithOpacity 
                                            value={getConfigString("overlayColor", "rgba(0,0,0,0.5)")}
                                            onChange={(c) => updateSectionResponsiveConfig("overlayColor", c)}
                                            label={`Overlay Color (${activeSectionDeviceTab.toUpperCase()})`}
                                        />
                                    </div>
                                    <div>
                                        <label className="text-[10px] text-[var(--fg-primary)] block mb-1">Background Size</label>
                                        <select 
                                            className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded p-1.5 text-xs outline-none focus:border-[var(--accent)]"
                    ...
                    <div className="space-y-6">
                        {/* Device Tabs for Spacing - Only Show if not already shown in Background & Padding */}
                        <div className="flex bg-[var(--bg-base)] p-1 rounded-lg mb-6 w-fit border border-[var(--border)]">
                            {devices.map((device) => (
                                <button
                                    key={device}
                                    onClick={() => setActiveSectionDeviceTab(device)}
                        ...
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)]">
                            <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Jarak Luar (Margin) - {activeSectionDeviceTab}</label>
                        ...
                        {/* Padding Settings */}
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)]">
                            <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Jarak Dalam (Padding) - {activeSectionDeviceTab}</label>
                            <div className="grid grid-cols-2 gap-3">
                                {/* Top */}
                                <div>
                                    <label className="text-[10px] text-[var(--fg-muted)] block mb-1">Atas (Top)</label>
                        ...
                        {/* Border Settings */}
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)]">
                            <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Border (Garis Batas) - {activeSectionDeviceTab}</label>
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <div>
                                     ...
                                     <select 
                                        className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                        value={getConfigString('borderStyle', 'solid')}
                                        onChange={(e) => updateSectionResponsiveConfig("borderStyle", e.target.value)}
                                     >
                                ...
                                <div>
                                     ...
                                     <ColorPickerWithOpacity
                                        label=""
                                        value={getConfigString("borderColor")}
                                        onChange={(c) => updateSectionResponsiveConfig("borderColor", c)}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="0"
                                        className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                        value={getConfigString('borderTopWidth')}
                                        onChange={(e) => {
                                            const val = parseInt(e.target.value);
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="0"
                                        className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                        value={getConfigString('borderBottomWidth')}
                                        onChange={(e) => {
                                            const val = parseInt(e.target.value);
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="0"
                                        className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                        value={getConfigString('borderLeftWidth')}
                                        onChange={(e) => {
                                            const val = parseInt(e.target.value);
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="0"
                                        className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                        value={getConfigString('borderRightWidth')}
                                        onChange={(e) => {
                                            const val = parseInt(e.target.value);
                        ...
                        <div className="bg-[var(--bg-surface)] p-4 rounded-xl border border-[var(--border)] mb-6">
                             <label className="text-xs font-bold text-[var(--fg-primary)] block mb-3">Bayangan (Shadow) - {activeSectionDeviceTab}</label>
                             <select 
                                className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--fg-primary)] rounded-lg p-2.5 text-sm focus:border-[var(--accent)] outline-none"
                                value={getConfigString('boxShadow', 'none')}
                                onChange={(e) => updateSectionResponsiveConfig("boxShadow", e.target.value)}
                             >
