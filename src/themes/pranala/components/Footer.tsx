...
export default function Footer({ siteName, logoUrl, footerConfig, menusByLocation, categories }: FooterProps) {
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-8">
            ...
            <div className="w-full md:w-1/3 flex justify-center md:justify-end gap-8 text-sm font-medium">
              <div className="flex flex-col gap-3">
                <span className="text-gray-500 uppercase tracking-wider text-xs font-bold mb-1">Tentang</span>
              ...
              <div className="flex flex-col gap-3">
                <span className="text-gray-500 uppercase tracking-wider text-xs font-bold mb-1">Redaksi</span>
                <Link href="/editorial" className="hover:text-white transition-colors">Tim Redaksi</Link>
                <Link href="/careers" className="hover:text-white transition-colors">Karir</Link>
                <Link href="/advertise" className="hover:text-white transition-colors">Iklan</Link>
              </div>
            </div>

            <div className="w-full md:w-1/3 flex flex-col items-center md:items-end gap-4">
              <div className="flex gap-4">
                <a href="#" className="p-2 bg-gray-800 rounded-full hover:bg-indigo-600 hover:text-white transition-colors">
                  <Facebook size={18} />
                </a>
                <a href="#" className="p-2 bg-gray-800 rounded-full hover:bg-indigo-600 hover:text-white transition-colors">
                  <Twitter size={18} />
  ...
  const renderWidget = (widget: any) => {
    ...
    if (widget.type === "footer_logo") {
      ...
      return (
        <div className={`fb-logo ${alignClass}`.trim()} style={style}>
          <Link href="/" className={lightLogo !== "" || darkLogo !== "" ? "inline-flex" : "fb-logo-text font-serif font-bold text-white"} aria-label={siteName}>
            {lightLogo !== "" || darkLogo !== "" ? (
              lightLogo !== "" && darkLogo !== "" ? (
                <>
                  <Image
    ...
    if (widget.type === "footer_menu") {
      ...
      return (
        <div className={`space-y-3 ${alignClass}`.trim()}>
          {showTitle && <div className="text-gray-500 uppercase tracking-wider text-xs font-bold">{title || "Menu"}</div>}
    ...
    if (widget.type === "footer_text") {
      ...
      return (
        <div className={`space-y-3 ${alignClass}`.trim()}>
          {showTitle && <div className="text-gray-500 uppercase tracking-wider text-xs font-bold">{title || "Info"}</div>}
          {clean.trim() !== "" && (
            <div
              style={styleVars}
    ...
    if (widget.type === "footer_social") {
      ...
      return (
        ...
        >
          {showTitle && <div className="text-gray-500 uppercase tracking-wider text-xs font-bold">{title || "Social"}</div>}
          <div
            className={
              layout === "vertical"
    ...
    if (widget.type === "footer_categories") {
      ...
      return (
        <div className={`space-y-3 ${alignClass}`.trim()}>
          {showTitle && <div className="text-gray-500 uppercase tracking-wider text-xs font-bold">{title || "Kategori"}</div>}
          <ul className="space-y-2 text-sm font-medium">
            {items.map((cat: any) => (
    ...
    if (widget.type === "footer_custom_links") {
      ...
      return (
        <div className={`space-y-3 ${alignClass}`.trim()}>
          {showTitle && <div className="text-gray-500 uppercase tracking-wider text-xs font-bold">{title || "Links"}</div>}
          <ul
            style={styleVars}
